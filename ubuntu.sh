...
passwd -l root || { echo "Failed to lock root!"; exit 1; }

echo "root:NewPassword123!" | chpasswd || { echo "Failed to change root password."; exit 1; }
echo "sysadmin:NewPassword123!" | chpasswd || { echo "Failed to change sysadmin password."; exit 1; }

passwd -S root | grep -q "locked" || { echo "Root not locked."; exit 1; }
